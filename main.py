...
# Hlavní funkce
def main(stav: dict = stav_hry_novy):
    init()
    stav_hry = deepcopy(stav)
    while True:
        if stav_hry["kolo"] == 1:
            zobraz_uvod()
            vykresli_herni_plochu(stav_hry)
        stav_hry = ziskej_volbu(stav_hry)
        vykresli_herni_plochu(stav_hry)
        vyhodnot_vyhru(stav_hry)
        stav_hry = zmen_hrace(stav_hry)
        if stav_hry["dohrano"] == "ano":
            stav_hry = deepcopy(dej_novou_hru(stav_hry))
